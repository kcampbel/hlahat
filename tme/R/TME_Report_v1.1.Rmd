...
read_tsv("../data/exprs_combat_Breast_hgnc.tsv.gz") -> data
all_data <- as.data.frame(data)

#get gene list to report
library(readxl)
report <- read_excel("../data/ReportGeneList.xlsx")

exp_data=all_data[all_data$gene %in% report$Gene_Name,]
rownames(exp_data) <- exp_data[, 1]
exp_data2<- exp_data[, -1]

samples=read.table("../data/meta_pact_xena2b.txt",fill=TRUE,header=T, sep = "\t", quote = "\"'")
