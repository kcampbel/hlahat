...
hotspots_goi <- hotspots %>%
  filter(!hgnc_symbol %in% goi_o$gene) %>%
  dplyr::select(gene=hgnc_symbol) %>%
  left_join(hgnc %>% dplyr::select(gene=approved_symbol, alias=alias_symbol)) %>%
  distinct()
hotspots_goi <- hotspots_goi[!duplicated(hotspots_goi$gene),]
