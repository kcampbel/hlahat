...
## HLA-HAT
The HLA Haplotype Analysis Toolkit assesses differences in allelic imbalance at the DNA and RNA level in Class I and II HLA loci.

-  For tumor DNA, allelic imbalance indicates a putative copy number aberration or loss-of-heterozygosity event at the gene or chromosomal level. 
-  For tumor RNA, allelic imbalance indicates differential expression between alleles.
```{r hlahat_stageinput}
...
### Summary
The differences in sequencing coverage of alleles are summarized using their ratio, the log-fold difference (LD). In DNA, we report the Copy Number Ratio (CNR), since the genetic imbalance reflects copy number differences at the DNA level. In the RNA, an imbalance represents differential expression of alleles. 
...
alleleld_tab <- dna_alleleld %>% filter(Gene %in% LETTERS[1:3] & median_ld>0) %>% 
  ...
  group_by(Gene) %>% 
  summarise(
    `DNA (CNR)` = paste0(gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 2`), " " , degree_dna, "  ", gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 1`), " (", signif(2^median_ld.dna, 2), ")"),
    `RNA (Ratio)` = case_when(median_ld.rna > 0 ~ paste0(gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 2`), " " , degree_rna, "  ", gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 1`), " (", signif(2^median_ld.rna, 2), ")"),
                    TRUE ~ paste0(gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 2`), " " , degree_rna, "  ", gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 1`), " (", signif(2^median_ld.rna, 2), ")"))
            ) 

title = dt_caption("Allelic imbalance for DNA and RNA. Values indicate the CNR (2^LD) for each locus.")
alleleld_tab %>%
  datatable(
    caption=title,
    extensions = dt_ext,
...
#### DNA: CNR vs Purity
The median copy number ratio (CNR) was calculated across all 26 HLA genes on Chromosome 6p and is summarized as a single value for the patient (shown by the red dot). The lines represent expected values of CNR based upon tumor purity.
...
comp_topur <- ggplot(exp_ald, aes(y = pur_cnr, x = pur, group = gen, colour = factor(cnr))) +
  ...
  labs(x = "Purity", y = "CNR", colour = "Expected CNR", fill = "Sample", title = "Observed CNR vs. Tumor Purity")  +
  theme_bw() +
