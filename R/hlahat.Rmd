...
dna <- dna_alleleld %>%
  dplyr::filter(ld_median > 0, Gene %in% genes)
rna <- rna_alleleld %>%
  dplyr::filter(ld_median > 0, Gene %in% genes)

plot_ly() %>%
  add_trace(data=dna, x=~Gene, y=~2^ld_median, type='bar', name='DNA',  marker = list(color = 'rgb(49,130,189)')) %>%
  add_trace(data=rna, x=~Gene, y=~2^ld_median, type='bar', name='RNA', marker = list(color = 'rgb(204,204,204)')) %>%
      layout(
             yaxis = list(title='Allelic imbalance')
      )
