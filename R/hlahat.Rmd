...
alleleld_tab <- dna_alleleld %>% filter(Gene %in% LETTERS[1:3] & ld_median>0) %>% 
  ...
  group_by(Gene) %>% 
  summarise(
    `DNA` = paste0(gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 2`), " " , degree_dna, "  ", gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 1`), " (", signif(2^ld_median.dna, 2), ")"),
    `RNA` = case_when(ld_median.rna > 0 ~ paste0(gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 2`), " " , degree_rna, "  ", gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 1`), " (", signif(2^ld_median.rna, 2), ")"),
                    TRUE ~ paste0(gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 2`), " " , degree_rna, "  ", gsub("(^\\w\\*\\d+:\\d+):*.+", "\\1", `Allele 1`), " (", signif(2^ld_median.rna, 2), ")"))
            ) 
...
```{r hlahat_dnarna_bar, fig.width=8, fig.height=4, eval=any(hla_abc %in% dna_alleleld$Gene)}
...
```{r hlahat_dnarna_box, fig.width=8, fig.height=4, eval=FALSE}
genes <- hla_abc[hla_abc %in% dna_alleleld$Gene]
dna <- dna_snpld %>%
  dplyr::filter(ld > 0, Gene %in% genes)
rna <- rna_snpld %>%
...
plot_ly() %>%
  add_trace(data=dna, x=~Gene, y=~ld, type='box', notched=TRUE, name='DNA', boxpoints='all', jitter=0.3, pointpos=0, 
    color = list(color = 'rgb(49,130,189)'), 
    marker = list(color = 'rgb(49,130,189)')) %>%
  add_trace(data=rna, x=~Gene, y=~ld, type='box', notched=TRUE, name='RNA', boxpoints='all', jitter=0.3, pointpos=0,
    color = list(color = 'grey'),
    marker = list(color = 'grey')) %>%
      layout(
             boxmode = 'group',
             yaxis = list(title='Allelic imbalance (log2)', range=c(0, max(df$ld) + 0.25))
      )
