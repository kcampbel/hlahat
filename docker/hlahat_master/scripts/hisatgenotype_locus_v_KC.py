...
import hisatgenotype_typing_common_v_KC as typing_common, hisatgenotype_assembly_graph as assembly_graph
# os.system("python /Users/katiecampbell/hisat_test/tm/hisat2-hisat2_v2.2.0_beta/hisatgenotype_modules/hisatgenotype_typing_common_KC10222019.py")
        ...
        for test_Gene_names in locus_list:
            ...
            if index_type == "graph":
                ...
                for line in alignview_proc.stdout:
                    ...
                    for cmp_i in range(len(cmp_list)):
                        ...
                        if type_ != "match" and type_ != "intron":
                            # Fails here on RNA: Introns do not have var_id's. If it is an intron, treat like a deletion
                        ...
                        if type_ != "deletion" and type_ != "intron":
                            read_pos += length_
            ...
            if base_fname == "hla":
                ...
                if perform_typing_primary_exon:
                    ...
                    # Incorporate representative alleles for exons PERFORMS EM TO GET ALLELIC ABUNDANCES
            ...
            for prob_i in range(len(Gene_prob)):
                ...
                if prob[1] < 0.01:
                    break
                found = False
                _allele_rep = prob[0] # KC allele probability
                """
                ...
                if simulation:
                    ...
                    if not False in found_list and prob_i >= 10:
                        break
                if not found:
                    for f_ in [sys.stderr, report_file]: # prints abundance percentage in report file
...
def genotyping_locus(base_fname,
    ...
    if genotype_genome == "": # SKIPS (genotype_genome provided)
    ...
    if genotype_genome != "": # KC THIS BLOCK OF CODE DOESNT MAKE SENSE TO ME
        if os.path.exists("%s.locus" % base_fname): # Doesnt complete because file is genotype_genome.locus not hla.locus
        # Wondering if base_fname and genotype_genome are switched
            left = 0
            Gene_genes = []
            BACKBONE = False
    ...
    if genotype_genome != "":
        ...
        if not typing_common.check_files(genome_fnames): # Checks to make sure all genotype_genome files exist
    ...
    if genotype_genome != "":
        for line in open("%s.allele" % genotype_genome):
            ...
            if family.lower() == base_fname: # family reads in as 'HLA' not 'hla'
    ...
    if genotype_genome != "":
        for line in open("%s.partial" % genotype_genome):
            family, allele_name = line.strip().split('\t')
            if family.lower() == base_fname: # added lower function to read family as 'HLA' not 'hla'
    ...
    else: # Added - if locus_list is specified, only assess specified genes
        refGene_loci = {key:refGene_loci[key] for key in locus_list}
    ...
    # Removed simulation for now (lines )
    if base_fname == "genome":
