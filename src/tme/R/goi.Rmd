...
vars_cols = c('gene', 'Protein_Change')
vars <- vars_o %>%
...
hotspots_any <- hotspots %>% 
  ...
  dplyr::select(gene, Protein_Change)
bm_hot_any <- bm %>%
  filter(gene %in% hotspots_any$gene,
         !is.na(Protein_Change)
  )

bm_hot_all <- rbind(bm_hot_pc, bm_hot_any)
keep <- c(goi_o$gene, bm_hot_all$gene)
bm_final <- bm %>% filter(gene %in% keep)
...
hotspot_tab <- bm_hot_all %>%
  inner_join(vars_o, by=c('gene', 'Protein_Change')) %>%
  arrange(gene) %>%
  dplyr::select(Gene=gene, `Mutation (CDS)`=cDNA_Change, `Mutation (AA)`=Protein_Change,`Class`=Variant_Classification, CNt=CN_total) 
